...
public class NetworkController : MonoBehaviourPunCallbacks
{
    ...
    [SerializeField] Text ChatInputText;
    ...
    public void SetNickName(string name)
    {
        ...
        //for (int i = 0; i < SetNickNameInputField.Length; i++)
        //{
        //    SetNickNameInputField[i].text = name;
        //}
    }
    ...
    {
        /*
        if (ChatInputText.IsActive() == false)
        {
        ...
        else
        {
            ...
            {
                FindObjectOfType<MyNetworkManager>().ChattedByPlayer(ChatInputText.text);
                ChatInputText.ActivateInputField();
                ChatInputText.Select();
            }
        }
        */
        ...
        Chat(PhotonNetwork.LocalPlayer.NickName + " : " + ChatInputText.text);
        ChatInputText.text = "";

        /*
        Character[] tmpPlayers = FindObjectsOfType<Character>();
        for (int i = 0; i < tmpPlayers.Length; i++)
        {
            if (tmpPlayers[i].GetComponent<PhotonView>().IsMine)
                tmpPlayers[i].GetComponent<PhotonView>().RPC("ChatRPC_Character", RpcTarget.All, msg);
        }
        */
    }
