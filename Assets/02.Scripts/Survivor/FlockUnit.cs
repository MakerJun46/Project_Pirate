    ...
    private void FindNeighbours()
    {
        ...
        for(int i = 0; i < allUnits.Length; i++)
        {
            ...
            if (currentUnit != this)
            {
                ...
                if (currentNeighbourDistanceSqr <= assignedFlock._cohesionDistance * assignedFlock._cohesionDistance)
                {
                    cohesionNeighbours.Add(currentUnit);
                }
                if (currentNeighbourDistanceSqr <= assignedFlock._avoidanceDistance * assignedFlock._avoidanceDistance)
                {
                    avoidanceNeighbours.Add(currentUnit);
                }
                if (currentNeighbourDistanceSqr <= assignedFlock._alignmentDistance * assignedFlock._alignmentDistance)
                {
